...
$$
	...
	=
	\sum_{i=1}^r \hslash^1_i(a_i,b_i)
	,
$$
where $\hslash^1_i(a,b)=1\otimes\cdots\otimes\hslash^1(a,b)\otimes\cdots\otimes 1$ is only non-identity in the $i$-th factor.
...
\begin{lemma}\label{lemma:sdrkoszul}\emph{(Simplification of Koszul factorizations by a higher order polynomial)}
	...
	\begin{tikzcd}[column sep=5.5cm]
		\displaystyle\bigoplus_{i=0}^{n-1}q^{-2i}\kmf{R}{\Res_f\mathbf{\overline a} & \Res_f\mathbf{\overline b}} 
		\arrow[r, "{\left(\sum_{j=1}^k\hslash^1_j(-\Quo_f\circ y^i a_j,-\Quo_f\circ y^i b_j)\right)_i}"] 
		& 
...
\begin{figure}
	...
	\begin{tikzcd}[row sep=0.5cm, column sep=1.5cm]
		&
		\scalebox{0.7}{\RLR{k}{n}{n}[4][1]}
		\arrow[r]
		\arrow[dr]
		&
		\scalebox{0.7}{\RLR{k}{k}{n}[5][2][$\hslash$]}
		\arrow[dr]
		&
		\\
		\scalebox{0.7}{\RLR{n}{n}{n}[3][0][$\hslash$]} 
		\arrow[dr]
		\arrow[r]
		\arrow[ur]
		&
		\scalebox{0.7}{\RLR{n}{k}{n}[4][1]}
		\arrow[ur, crossing over]
		\arrow[dr]
		& 
		\scalebox{0.7}{\RLR{k}{n}{k}[5][2][$\hslash$]}
		\arrow[r]
		& 	
		\scalebox{0.7}{\RLR{k}{k}{k}[6][3]}
		\\
		& 
		\scalebox{0.7}{\RLR{n}{n}{k}[4][1]} 
		\arrow[ur, crossing over]
		\arrow[r]
		&
