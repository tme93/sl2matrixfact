...
\end{tikzcd}
\]
Many useful maps can be easily expressed using this notation. In the case $M'=\hslash M$, we will often use the map
$$
s_\hslash =\hslash^1(1,1):M\to \hslash M
.
$$
\subsection{Koszul matrix factorizations}
For Koszul matrix factorizations, we will take the convention
$$
	\kmf{R}{a & b }
	=
	\left[\mfshort{q^{\frac12 (\deg_q(a)-\deg_q(b))} \hslash  R}{R}{a}{b}\right]
	.
$$
As a module, we think of  $\kmf{R}{a & b }$ as having a basis $\left\{\widehat a,\widehat b\right\}$, with $\widehat a$ in degree $q^{\frac12 (\deg_q(a)-\deg_q(b))} \hslash$ and $\widehat b$ in degree $q^0$,
so that the differential can be written as
$$
d_{\skmf{R}{a & b }}=\begin{pmatrix}
	0 & b \\ a & 0
\end{pmatrix}=\hslash^1 (a,b)
$$
More generally, given sequences $\mathbf{\overline a}=(a_1,\dots,a_r)^t$ and  $ \mathbf{\overline b}=(b_1,\dots,b_r)^t$, we have an associated Koszul factorization
...
$$
For any shift of a Koszul matrix factorization, say $q^k h^j\hslash^\sigma\kmf{R}{\mathbf{\overline a} & \mathbf{\overline b} }$, the choice of $\mathbf{\overline a}$ and $\mathbf{\overline b}$ determines a basis of the entire tensor product: $\left\{\theta_1\otimes\cdots\otimes\theta_r\mid \theta_i=\widehat a_i \mbox{ or } \widehat b_i\right\}$, where the degree of $\widehat b_1\otimes\cdots\otimes\widehat b_r$ is $q^kh^j\hslash^\sigma$, while the other basis elements are shifted according to which $\widehat a_i$ appear in a given basis element. 
%Denote $\overline\theta_i=\widehat a_i$ or $\widehat b_i$, opposite to which $\theta_i$ is. 

Given $p,q\in R$, and two shifted Koszul factorizations of the same length, we will often use maps 
$$
\hslash^\sigma_i(p,q)
:
q^k h^j\hslash^\sigma\kmf{R}{\mathbf{\overline a} & \mathbf{\overline b} }
\longrightarrow
q^{k'} h^{j'}\hslash^{\sigma'}\kmf{R}{\mathbf{\overline a}' & \mathbf{\overline b}' }
$$
that amount to applying $\hslash^\sigma(p,q)$ at the $i$-th tensor component. Let us use our basis to  make this precise. Here we use the same notation $\theta_i$ for the bases on the domain and codomain.
\begin{align*}
	\hslash^0_i(p,q)(\theta_1\otimes\cdots\otimes \theta_r)=
	\begin{cases}
		p\theta_1\otimes\cdots\otimes \widehat a_i\otimes\cdots\otimes \theta_r, & \mbox{if }\theta_i=\widehat a_i
		\\
		q\theta_1\otimes\cdots\otimes \widehat b_i\otimes\cdots\otimes \theta_r, & \mbox{if }\theta_i=\widehat b_i
	\end{cases}
	\\
		\hslash^1_i(p,q)(\theta_1\otimes\cdots\otimes \theta_r)=
	\begin{cases}
		(-1)^{\sum_{j<i}\deg_{\hslash}(\theta_j)}p\theta_1\otimes\cdots\otimes \widehat b_i\otimes\cdots\otimes \theta_r, & \mbox{if }\theta_i=\widehat a_i
		\\
		(-1)^{\sum_{j<i}\deg_{\hslash}(\theta_j)}q\theta_1\otimes\cdots\otimes \widehat a_i\otimes\cdots\otimes \theta_r, & \mbox{if }\theta_i=\widehat b_i
	\end{cases}
\end{align*}
The sign in $\hslash^1_i(p,q)(\theta_1\otimes\cdots\otimes \theta_r)$ can also be formulated as the parity of $|\left\{ j \mid j<i, \theta_j=\widehat a_j\right\}|$.




With this notation, the differential can be written as
$$
	d_{q^ih^j\hslash^\sigma\skmf{R}{\mathbf{\overline a} & \mathbf{\overline b} }}
	=
	(-1)^\sigma\sum_{i=1}^r \hslash^1_i(a_i,b_i)
	.
$$

 We have following properties, which follow easily from the definitions:
\begin{lemma}\label{hslashformulas} The following identities hold.
\begin{align*}
	\hslash_i^\sigma(p,q)\hslash_j^\rho(c,d)&=(-1)^{\sigma\rho}\hslash_j^\rho(c,d)\hslash_i^\sigma(p,q), &i\neq j
	\\
	\hslash_i^\sigma(c,d)\hslash_i^0(p,q)&=\hslash_i^\sigma(cp,dq)
		\\
	\hslash_i^\sigma(p,q)\hslash_i^1(c,d)&=\hslash_i^{1-\sigma}(qc,pd)
\end{align*}
\end{lemma}

\begin{remark}
	The definition of $\hslash_i^\sigma(p,q)$ depends on the basis, which ultimately depends on the $\mathbf{\overline a}$ and  $ \mathbf{\overline b}$ used to present the matrix factorization. For example, the same matrix factorization could be written in two different ways:
		\begin{align*}
		\kmf{R}{f & g} 
		&= q^{\frac12 (\deg_q(f)-\deg_q(g))}\hbar \kmf{R}{-g & -f},
	\end{align*}
and the roles of $\widehat a$ and $\widehat b$ switch from one expression to the other. What we would denote $\hslash_1^\sigma(p,q)$ with domain $\kmf{R}{f & g} $, becomes $\hslash_1^{1-\sigma}(q,p)$ when the domain is $q^{\frac12 (\deg_q(f)-\deg_q(g))}\hbar \kmf{R}{-g & -f}$ instead. 
\end{remark}


\begin{lemma}
	Let $\mathbf{\overline a}'$ and $\mathbf{\overline b}'$ be related to $\mathbf{\overline a}$ and $\mathbf{\overline b}$ by
	$$
	a'_j=\begin{cases}
		a_j, & j\neq i \\ -b_i, & j=i,
	\end{cases}\qquad b'_j=\begin{cases}
		b_j, & j\neq i \\ -a_i, & j=i, 	\end{cases}
	$$
Then we have an isomorphism	
	$$\id=\hslash^1_i(1,1):\kmf{R}{\mathbf{\overline a} &\mathbf{\overline b} } \longrightarrow	q^{\frac12 (\deg_q(a_i)-\deg_q(b_i))}\kmf{R}{\mathbf{\overline a}' &  \mathbf{\overline b}' }.$$
\end{lemma}

When tensoring a Koszul factorization $K_{R}\begin{pmatrix} a & b \end{pmatrix}$ with some other matrix factorization $M$, we sometimes write
\begin{equation}\label{koszulexpand}
	\kmf{R}{a & b }\otimes_R M
	=
	\left[ \mfshort{q^{\frac12 (\deg_q(a)-\deg_q(b))}\hslash M}{M}{a}{b} \right]
	.
\end{equation}
...
Let write down some useful isomorphisms.

\begin{lemma} Let $\lambda\in R$. The following are isomorphisms with matrix factorizations.
	\begin{align*}
		l_{i,j}(\lambda)=\id+\lambda \hslash^1_i(1,0)\hslash^1_j(1,0):\kmf{R}{\mathbf{\overline a} &\mathbf{\overline b} }  
		&\longrightarrow
		\kmf{R}{\mathbf{\overline a}-\lambda b_i\mathbf{\overline e}_j+\lambda b_j\mathbf{\overline e}_i &  \mathbf{\overline b} }
		\\
		m_{i,j}(\lambda)=\id+\lambda \hslash^1_i(0,1)\hslash^1_j(1,0):\kmf{R}{\mathbf{\overline a} & \mathbf{\overline b} }  
		&\longrightarrow
...
\begin{proof}
First, we verify that $l_{i,j}(\lambda)$ is a morphism of matrix factorizations by verifying
	$$l_{i,j}(\lambda)\sum_k \hslash^1_k(a_k,b_k)= \left(\lambda(\hslash^1_i(b_j,0)-\hslash^1_j(b_i,0))+\sum_k\hslash^1_k(a_k,b_k)\right)l_{i,j}(\lambda)$$
	Since $l_{i,j}(\lambda)$ commutes with $\sum_{k\neq i,j} \hslash^1_k(a_k,b_k)$, it reduces to
	$$l_{i,j}(\lambda)\left(\hslash^1_i(a_i,b_i)+\hslash^1_j(a_j,b_j)\right)= \left(\hslash^1_i(a_i+\lambda b_j,b_i)+\hslash^1_j(a_j-\lambda b_i,b_j)\right)l_{i,j}(\lambda),$$
and expanding both sides and cancelling identical terms reduces once again to
\begin{align*}\lambda \hslash^1_i(1,0)\hslash^1_j(1,0)\left(\hslash^1_i(a_i,b_i)+\hslash^1_j(a_j,b_j)\right)&= \left(\hslash^1_i(a_i+\lambda b_j,b_i)+\hslash^1_j(a_j-\lambda b_i,b_j)\right)\lambda \hslash^1_i(1,0)\hslash^1_j(1,0)\\&\phantom{=}+\hslash^1_i(\lambda b_j,0)+\hslash^1_j(-\lambda b_i,0)
\end{align*}
Using the formulas in Lemma \ref{hslashformulas}, the left and right hand side become
\begin{align*}\lambda\left(-\hslash^1_i(0,b_i)\hslash_j^1(1,0)+\hslash^1_i(1,0)\hslash_j^1(0,b_i)\right)&= \lambda\left(\hslash^1_i(b_i,0)\hslash^1_j(1,0)-\hslash^1_i(1,0)\hslash^1_j(b_j,0)+\hslash^1_i(b_j,0)-\hslash^1_j(b_i,0)\right).
\end{align*}
This last identity is true because
\begin{align*}
	-\hslash^1_i(0,1)\hslash_j^1(1,0)&= \hslash^1_i(1,0)\hslash^1_j(1,0)-\hslash^1_j(1,0)
	\\
	\hslash^1_i(1,0)\hslash_j^1(0,1) &=-\hslash^1_i(1,0)\hslash^1_j(1,0)+\hslash^1_i(1,0)
\end{align*}
To prove that $l_{i,j}(\lambda)$ is an isomorphism, we simply observe that $l_{i,j}(\lambda)^{-1}=l_{i,j}(-\lambda)$. The case of $m_{i,j}$ is completely analogous.
\end{proof}

\begin{lemma}
If $\mathbf{\overline a}'$ and $\mathbf{\overline b}'$ are obtained from $\mathbf{\overline a}$ and $\mathbf{\overline b}$ respectively by swapping the  $i$-th and $j$-th entries: $a_i\leftrightarrow a_j$ and $b_i\leftrightarrow b_j$. Then we have a swap isomorphism $\tau_{i,j}:\kmf{R}{\mathbf{\overline a} &\mathbf{\overline b} } \to	\kmf{R}{\mathbf{\overline a}' &  \mathbf{\overline b}' }$ of the form
\begin{align*}
	\tau_{i,j}=\hslash^0_i(1,0)\hslash^0_j(1,0)+\hslash^0_i(0,1)\hslash^0_j(0,1)+\hslash^1_i(1,0)\hslash^1_j(0,1)-\hslash^1_i(0,1)\hslash^1_j(1,0). 
\end{align*}
\end{lemma}
...
\begin{align*}
	...
	\longmapsto \maf(D_1)&=\left[\mf{\hslash R}{q R}{(x_0+x_2-x_1-x_3)(x_0-x_2)}{(x_3-x_1)-H}\right]
	\\
...
\end{align*}
In all types of thin edges, there is a \emph{negative region} whose variable appears with a negative sign in the linear term of the associated matrix factorization. Thus, all thin and thick edges come with an orientation, so that the negative region is the one on the left relative to such orientation. In the case of $D_0$ and $D_1$, there are already canonical orientations
\todo{orientations for $D_0$ and $D_1$}
and in these cases the arrows are omitted.


For $D_2$ and $D_3$, there is a 180 degree rotational symmetry preventing a canonical choice of orientation, so the arrow is always indicated.

Notice that $R$ is the subring of the polynomial ring in all face variables generated by differences across edges. We refer to $R$ as the \emph{edge ring}. The subring $R'$ generated by differences across boundary-adjacent edges is called the \emph{boundary ring}, and $R=R'$ if and only if there are no internal faces. We extend this nomenclature to more general webs in the expected way. The particular case $R'=\mathbf K[x_0-x_1,x_2-x_1,x_3-x_2]$ is refered to as the \emph{standard boundary ring}. We will often omit the specification of the edge or boundary ring when face variables are already indicated in the web diagram.

Now, let $\Gamma$ be an arbitrary connected web on the disk. By this, we mean an embedded directed graph, considered up to isotopy, with 1-valent vertices only possibly at the boundary of the disk, and interior 3-valent vertices only possibly at the interior. Each 3-valent vertex must have a neighborhood looking like one of the $D_i$, possibly with a reversal of thin or thick edge orientation. We associate a matrix factorization
$$
...
For convenience, we often represent Koszul matrix factorizations associated to webs by the web itself with some decorations, namely
\begin{itemize}
	\item labels indicating the region variables and
	\item surrounding indices indicating overall degree shifts.
\end{itemize}

There is an implicit convention for the ordering of the thin and thick edges: an edge on top comes before an edge on bottom, and then left comes before right if their centers are in the same horizontal line.

For instance, if $\Gamma$ is the web with one thin edge and two thick edges as depicted, then


\begin{align*}
q^ih^j\hslash\maf(\Gamma) &=\text{\LRL{nr}{kr}{n}[$i$][$j$][$\hslash$][1]}
\\&=
q^{i+1}h^j\hslash\kmf{R[u-x_1]}{-(x_0-u) & -(x_0+u-x_5-x_1)((x_5-x_1)-H) \\
	-(x_5+x_3-x_4-u)(x_5-x_3)& -((x_4-u)-H)  \\ 	
	(u-x_2)& (u+x_2-x_3-x_1)((x_3-x_1)-H)
}
\end{align*}
as a matrix factorization with boundary ring $R'=\mathbf K[x_0-x_1,x_2-x_1,x_3-x_2,x_4-x_3,x_4-x_5]$ and edge ring $R=R'[u-x_1]$. The additional $q^1$ shift comes from the shift already present in the definition of the matrix factorization associated to the thick edge.

\begin{example}\label{example:tripleedge}
	According to our conventions, we have
	\begin{align*}\label{example:rlthick}
		N_1:=\RLR{k}{k}{k}[-3][][][1]
		&
		=
		\kmf{R[u-x_1]}{(x_5+u-x_4-x_0)(x_5-u) & (x_4-x_0)-H \\
			(x_0+x_2-u-x_1)(x_0-x_2)& (u-x_1)-H  \\ 	
			(u+x_3-x_4-x_2)(u-x_3)& (x_4-x_2)-H 
		}
	\end{align*}
This is the Koszul matrix factorization obtained by tensoring the contributions of each of the three thick edges, with a correcting shift by $q^{-3}$. Expanding as in (\ref{koszulexpand}), we can write

$$N_1=\left[\begin{tikzcd}[column sep=3.5cm]
	\scalebox{0.8}{\LR{k}{k}[-3][][$\hslash$][2]}[u-x_5] 
	\arrow[r, "(x_5+u-x_4-x_0)(x_5-u)", bend left=5]
	& 
	\scalebox{0.8}{\LR{k}{k}[-2][][][2]}[u-x_5] 
	\arrow[l, "(x_4-x_0)-H", bend left=5] 
\end{tikzcd}\right]$$
Let use this type of expansion to represent the swap map between the first two rows of $N_1$. Denoting by $N_2$ the matrix factorization obtained after the swap, we can write
$$N_2:=\left[\begin{tikzcd}[column sep=3.5cm]
	\scalebox{0.8}{\RER{k}{k}[-3][][$\hslash$][2]}[u-x_1] 
	\arrow[r, "(x_0+x_2-u-x_1)(x_0-x_2)", bend left=5]
	& 
	\scalebox{0.8}{\RER{k}{k}[-2][][][2]}[u-x_1] 
	\arrow[l, "(u-x_1)-H", bend left=5]
	.
\end{tikzcd}\right]$$
The swap map $\tau_{1,2}:N_1\to N_2$ is

$$
\begin{tikzcd}[column sep=3.5cm, row sep=1cm]
	\scalebox{0.8}{\LR{k}{k}[-3][][$\hslash$][2]}[u-x_5] 
	\arrow[r, "(x_5+u-x_4-x_0)(x_5-u)", bend left=5]
	\arrow[d, "{\hslash_1^0(1,0)}", swap]
	\arrow[dr, "{\hslash_1^1(0,1)}"{pos=0.9}, out=-30, in=150]
	& 
	\scalebox{0.8}{\LR{k}{k}[-2][][][2]}[u-x_5] 
	\arrow[l, "(x_4-x_0)-H", bend left=5]
	\arrow[d, "{\hslash_1^0(0,1)}"]
	\arrow[dl, "{-\hslash_1^1(1,0)}"{pos=0.9}, out=-150, in=30, swap]
	\\
	\scalebox{0.8}{\RER{k}{k}[-3][][$\hslash$][2]}[u-x_1] 
	\arrow[r, "(x_0+x_2-u-x_1)(x_0-x_2)", bend left=5]
	& 
...
In this section, we explain how to express matrix factorizations associated to more complex webs in terms of simpler webs. For the most part, this entails the reduction of webs that have internal faces into webs with less or no internal faces. In particular, these are situations where the edge ring $R$ is larger than the boundary ring $R'$. Example \ref{example:tripleedge}, for instance, involved matrix factorizations $N_i$ over $R=R'[u-x_1]$, where $u$ is the variable labeling the internal face.  Since the potential itself is an element of the boundary ring $R'$ (all the terms involving $u$ cancel out), it is possible to think of $N_i$ as matrix factorizations over $R'$, albeit one of infinite rank. 

The following lemma gives us a recipe to obtain a finite rank special deformation retract in that scenario by eliminating the first row and taking a quotient.
\end{example}

\begin{lemma}\label{sdrlemma}    (Adapted from KR \textcolor{red}{How general is $R$?}) Let $S$ be an integral domain and $y$ a variable in quantum degree $q^{-2}$. Let $\overline W\in S$ and $f,g\in S[y]$ so that $f$ has the form $f=uy^n+\tilde f$ wtih $u\in S^\times$ and $\deg_y(\tilde f)<n$. Let $g$ have degree $q^{-2m}$. Let $M$ be a matrix factorization over $S[y]$ with potential $W=\overline W-fg\in S[y]$. Denote
\begin{align*}
	M'
	&
	=M/fM
	...
	&
	= 
	\kmf{S[y]}{f & g}\otimes_{S[y]} M
	,
\end{align*}
thought of as matrix factorizations over $S$ with potential $\overline W$.
...
\begin{proof}
	...
	$$ 
	is verified as follows. Take $(x,y)\in q^{m-n}\hslash M \oplus M=\overline M$. We compute directly, keeping in mind that $\Res_f(z)=z-f\Quo_f(z)$ and $\Quo_f(fz)=z$:
	\begin{align*}
...
\begin{remark}
	There is a slight variation in which $f$ appears in the second column
	$$
		\overline M 
		= 
		\kmf{S[y]}{g & f}\otimes_{S[y]} M.
	$$
	In this case, we have $M'=q^{n-m}\hslash M/fM$, and the strong defomation retract has the form 
	$$
	\begin{tikzcd}[column sep=large, ]
		q^{n-m}\hslash M/fM
		\ar[dd, shift right=0.6ex, "I_v", swap]
		\ar[dd, leftarrow, shift left=0.6ex, "P"]
		\arrow[ddr, "I_d", bend left=20, swap]
		&
		\\
		\\
		q^{n-m}\hslash M
		\arrow[r, "g", shift left=1ex, bend left=20, start anchor=east, end anchor=west]
		\ar[r, "K", swap]
		& 
		M
		\arrow[l, "f", shift left=1ex, bend left=20, start anchor=west, end anchor=east], 
	\end{tikzcd}
	$$
\end{remark}
\begin{example}
	Let us return to example \ref{tripleedge} and apply the lemma to the matrix factorization $N_2$. Let us denote $y=u-x_1$, $f=(u-x_1)-H$ and
	\begin{align*}
		M
		&
		=
		\scalebox{0.8}{\RER{k}{k}[-2][][][2]}[u-x_1]  
		=
		\kmf{R}{(x_5+u-x_4-x_0)(x_5-u) & (x_4-x_0)-H \\		
			(u+x_3-x_4-x_2)(u-x_3)& (x_4-x_2)-H 
		}
		,
	\end{align*}
so that $N_2=\kmf{R}{g & f}\otimes_{R} M $. Thus, $N_2$ admits a deformation retract of the form
	\begin{align*}
	q^{-1}\hslash M/fM
	&
	=
	q^{-1}\hslash\kmf{R'}{(x_5+x_1+H-x_4-x_0)(x_5-H-x_1 ) & (x_4-x_0)-H   \\	
		(x_1+H+x_3-x_4-x_2)(x_1+H-x_3)& (x_4-x_2)-H 
	}
.
\end{align*}

Now that we have a smaller factorization $M/fM$ over a smaller ring $R'$, we would like to identify it as the matrix factorization associated to a web. We can do this through some elementary operations: 
	\begin{align*}
	q^{-1}\hslash M/fM
%	=
%	\kmf{R'}{H-(x_4-x_0) & (x_5+x_1+H-x_4-x_0)(x_1+H-x_5) \\	
%		(x_1+H+x_3-x_4-x_2)(x_1+H-x_3)& (x_4-x_2)-H 
%	}
%	\\
%	&
...
%	\\
		&
	\xrightarrow{\xi: =\hslash^1_1(1,1)\circ l_{1,2}(x_5-x_1-H)\circ m_{1,2}(-1)}
%	q\kmf{R}{x_0-x_2 & (x_5+x_1-x_2-x_0)(x_1+H-x_5) \\	
%	(x_5-x_3)(x_5+x_3-x_4-x_2)& (x_4-x_2)-H 
%	}
%	\\
%	&=
	\scalebox{0.8}{\LR{n}{k}[-3]}
\end{align*}
We have enough formulas to calculate an explicit strong deformation retract
\[
\begin{tikzcd}[column sep=large, ampersand replacement=\&]
	{\scalebox{0.8}{\LR{n}{k}[][][][1]}} 
	\arrow[r, shift left=0.5ex, color=violet] 
	\arrow[r, leftarrow, shift right=0.5ex, swap, color=violet] 
	\& 
	{\scalebox{0.8}{\RLR{k}{k}{k}[][][][1]}} 
	\arrow[loop right, distance=1em, in=15, out=-15, ,swap, color=violet]
\end{tikzcd}
\]
but for our purposes, only the retraction(right-to-left) will be needed in this example. This a composite
\[
...
We simplified a Koszul matrix factorization by killing a linear polynomial in this example. This procedure will be applied often, so we record it as a more general lemma. 
...
\begin{lemma}\label{lemma:sdrkoszullinear} \emph{(Simplification of Koszul factorizations by a linear term)}
	...
	\begin{tikzcd}[column sep=large]
		...
		\arrow[ddl, "{-\sum_{j=1}^k\hslash_j^1(\Quo_f\circ a_j,\Quo_f\circ b_j)}", bend right=20, swap]
		\\\\
		q^{m-1}\hslash\kmf{R[y]}{\mathbf{\overline a} & \mathbf{\overline b}}
	...
	\end{tikzcd}
	$$
	where $g$ has degree $q^{-2m}$.
%	The diagonal component of the retraction is the composite
%	$$
%	\begin{tikzcd}
%		\kmf{R}{\left.\mathbf{\overline a}\right|_{y=f_0} & \left.\mathbf{\overline b}\right|_{y=f_0}}
%		\arrow[r, "s_\hslash"]
%		&[-0.3cm]
%		\hslash \kmf{R}{\left.\mathbf{\overline a}\right|_{y=f_0} & \left.\mathbf{\overline b}\right|_{y=f_0}} 
%		\arrow[r, "{-\sum_{j=1}^k\hslash_j^1(\Quo_f\circ a_j,\Quo_f\circ b_j)}"] 
%		&[3cm]
...
In other cases of interest, we have a decomposition into several summands $M/fM=\bigoplus_{i=0}^{k}M_i$ as matrix factorizations over $R$, where $M_i$ are simpler factorizations of a desirable form, e.g. coming from simpler webs. The following lemma, a higher order version of \ref{lemma:sdrkoszullinear}, gives such decompositions in favorable cases.
...
\begin{lemma}\label{lemma:sdrkoszul}\emph{(Simplification of Koszul factorizations by a higher order polynomial)}
	...
	$$
		\begin{tikzcd}[column sep=large, ]
			...
			\arrow[ddl, "{-\left(\sum_{j=1}^k\hslash^1_j(\Quo_f\circ y^i a_j,\Quo_f\circ y^i b_j)\right)_i}", bend right=20, swap]
			\\
			\\
			q^{m-n}\hslash \kmf{R[y]}{\mathbf{\overline a} & \mathbf{\overline b}}
			\arrow[r, "f", shift left=0.5ex, bend left=20]
			& 
			\kmf{R[y]}{\mathbf{\overline a} & \mathbf{\overline b}}
			\ar[l, "-\Quo_f"]
		...
		\end{tikzcd}
	$$
%	The diagonal component of the inclusion is
%	$$
%	\begin{tikzcd}[column sep=6cm]
%		\displaystyle\bigoplus_{i=0}^{n-1}q^{-2i}\kmf{R}{\Res_f\mathbf{\overline a} & \Res_f\mathbf{\overline b}} 
%		\arrow[r, "{-\left(\sum_{j=1}^k\hslash^1_j(\Quo_f\circ y^i a_j,\Quo_f\circ y^i b_j)\circ s_\hslash\right)_i}"] 
%		& 
%		q^{m-n}\hslash K_{R[y]}\left(\mathbf{\overline a},\mathbf{\overline b}\right)
%	\end{tikzcd}
%	$$
\end{lemma}
...
\begin{proposition}
	...
	\begin{tikzcd}[column sep=2cm]
		...
		\arrow[ddl, "{-\left(\sum_{j}\hslash^1_j(\Quo_fa_j,\Quo_fb_j)\right)}", bend right=20, swap]
		\\
		\\
...
\begin{example}
	In the proof of the Reidemeister 3 move we will need a special deformation retract of the form
	\[
	\begin{tikzcd}[column sep=large, ampersand replacement=\&]
		{\scalebox{0.7}{\LR{k}{k}[][][][1]}} 
		\arrow[r, shift left=0.5ex, color=violet] 
		\arrow[r, leftarrow, shift right=0.5ex, swap, color=violet] 
		\& 
		{\scalebox{0.7}{\RLR{n}{k}{k}[][][][1]}} 
		\arrow[loop right, distance=1em, in=15, out=-15, ,swap, color=violet]
	\end{tikzcd}
	\]
...
\todo{signs need double check here}
	...
	\begin{tikzcd}[column sep=large]
		...
		\ar[ddl, "{\hslash^1_1(x_0-x_2,-1)+\hslash^1_2(x_2+x_4-u-x_5,0)}", bend right=20, swap, color=violet]
		\\
		\\
		\scalebox{0.7}{\LR{k}{k}[1][][$\hslash$][2]}[x_5-u]
...
\[
\begin{tikzcd}[column sep=2cm,ampersand replacement=\&]
	\scalebox{0.7}{\SingleEdge{}{}[2]}\oplus \scalebox{0.7}{\SingleEdge{}{}}   
	\arrow[r, "{\begin{pmatrix} \hslash^0(1,0)&\hslash^0(x_3-x_0,0)
	\end{pmatrix}}"] 
	\&[1.2cm]
	\scalebox{0.7}{\ThinEdgeAndCircle{}{}{}[1][][$\hslash$]} 
	\arrow[r, "{\hslash^1(1,2x_0-x_1-x_3)}"]
	\&
	\scalebox{0.7}{\ThickEdgeAndCircle{}{}{}[2][1]}
	\arrow[r, "{\hslash^0(0,P^0_{f(y)})}", "\scalebox{0.6}{$\begin{matrix}
			y=x_3-x_0 \\ f(y)= x_3-x_0
		\end{matrix}$}"']
	\&[-0.4cm]
	\scalebox{0.7}{\SingleEdge{}{}[2][1][$\hslash$]} 
\end{tikzcd}
\]
...
\begin{tikzcd}[column sep=2cm, ampersand replacement=\&]
	...
	\&[-0.7cm]
	\scalebox{0.7}{\ThickEdgeAndCircle{}{}{}[-2][-1][$\hslash$]}
	\arrow[r, "{\hslash^1(1,2x_0-x_1-x_3)}"]
	\&[0.5cm]
	\scalebox{0.7}{\ThinEdgeAndCircle{}{}{}[-1]} 
	\arrow[r, "{\begin{pmatrix} \hslash^0(P^0_{f(y)},0) \\
		...
		\end{matrix}$}"']
	\&[0.3cm]
	\scalebox{0.7}{\SingleEdge{}{}}\oplus \scalebox{0.7}{\SingleEdge{}{}[-2]}
\end{tikzcd}
\]
...
The summands 
$$
\left[\scalebox{0.7}{\SingleEdge{}{}[2]}
\xrightarrow{\hslash^1(1,0)} 
\scalebox{0.7}{\SingleEdge{}{}[2][1][$\hslash$]]}\right]
\qquad \mbox{and}\qquad 
\left[\scalebox{0.7}{\SingleEdge{}{}[-2][-1][$\hslash$]}
\xrightarrow{\hslash^1(0,-1)} 
\scalebox{0.7}{\SingleEdge{}{}[-2]}\right]
$$
...
\begin{figure}
	\centering
	\[\label{r1}
	\begin{tikzcd}[column sep=2.5cm, ampersand replacement=\&]
		...
		\\
		\scalebox{0.7}{\ThinEdgeAndCircle{}{}{}[1][][$\hslash$]} \arrow[r]
	...
	\begin{tikzcd}[column sep=1.8cm]
		...
		\arrow[d, color=ForestGreen, shift right=1.5] 
		\\
		\scalebox{0.7}{\SingleEdge{}{}[-2][-1][$\hslash$]}   \arrow[d, color=violet, shift right=1.5] 
...
\begin{figure}
	...
	\begin{tikzcd}[row sep=-0.1cm, column sep=2cm]
		&
		\scalebox{0.7}{\UnorientedVerticalThinEdge{}{}{}{}{-Stealth}}
		\arrow[d, color=ForestGreen, shift right=1.5]
		&
		\\[4em]&
		\scalebox{0.7}{\UnorientedVerticalThinEdge{}{}{}{}{-Stealth}}
...
\begin{figure}
	\centering
	\[\label{r3sdr}
	\begin{tikzcd}[row sep=0.5cm, column sep=3cm]
		&
		\scalebox{0.7}{\RLR{k}{nr}{n}[4][1]}
		\arrow[r]
		\arrow[dr]
		&
		\scalebox{0.7}{\RLR{kr}{kr}{n}[5][2][$\hslash$]}
		\arrow[dr, "{\hslash_3^1(u+x_3-x_4-x_2,1)}"]
		&
		\\
		\scalebox{0.7}{\RLR{n}{nr}{n}[3][0][$\hslash$]} 
		\arrow[ur, "{\hslash_1^1(x_5+u-x_4-x_0,1)}"]
		\arrow[r, "{\hslash_2^1(x_0+x_1-u-x_1,1)}"]
		\arrow[dr, "{\hslash_3^1(u+x_3-x_4-x_2,1)}", swap]
		&
		\scalebox{0.7}{\RLR{nr}{kr}{n}[4][1]}
		\arrow[ur, crossing over]
		\arrow[dr]
		& 
		\scalebox{0.7}{\RLR{kr}{n}{k}[5][2][$\hslash$]}
		\arrow[r, "{\hslash_2^1(x_0+x_1-u-x_1,1)}"]
		& 	
		\scalebox{0.7}{\RLR{k}{k}{k}[6][3]}
		\\
		& 
		\scalebox{0.7}{\RLR{nr}{n}{k}[4][1]} 
		\arrow[ur, crossing over]
		\arrow[r]
		&
		\scalebox{0.7}{\RLR{n}{k}{k}[5][2][$\hslash$]}
		\arrow[ur, "{\hslash_1^1(x_5+u-x_4-x_0,1)}", swap]
		&
	\end{tikzcd}
	\]
	\caption{Invariance under Reidemeister III move}
\end{figure}
...
\begin{figure}
	...
	\begin{tikzcd}[row sep=0.5cm, column sep=1.5cm]
		&
		\scalebox{0.6}{\RL{k}{n}[4][1]}
		\arrow[r]
		\arrow[dr]
		&
		\scalebox{0.6}{\RL{k}{k}[5][2][$\hslash$]}
		\arrow[dr]
		&
		\\
		\scalebox{0.6}{\LR{n}{n}[3][0][$\hslash$]} 
		\arrow[dr]
		\arrow[r]
		\arrow[ur]
		&
		\scalebox{0.6}{\LR{k}{n}[4][1]}
		\arrow[ur, crossing over]
		\arrow[dr]
		& 
		\scalebox{0.6}{\LR{n}{k}[5][2][$\hslash$]}\oplus\scalebox{0.6}{\LR{n}{k}[3][2][$\hslash$]}
		\arrow[r]
		& 	
		\scalebox{0.6}{\LR{n}{k}[6][3]}
		\\
		& 
		\scalebox{0.6}{\LR{n}{k}[4][1]} 
		\arrow[ur, crossing over]
		\arrow[r]
		&
		\scalebox{0.6}{\LR{k}{k}[5][2][$\hslash$]}
		\arrow[ur]
		&
	\end{tikzcd}
	\]
